...
track, not a solved problem. This 14--18$\times$ figure is the
...
by 11--342$\times$ (Bouc-Wen: 26--42$\times$, within the same range). This
...
family (\texttt{backlash}) throughout Sections~\ref{sec:headline}--\ref{sec:identifiability}. We ran the full leave-one-family-out sweep flagged as future
work in an earlier draft of this paper: training on each of the other four
families as the sole held-out choice (5 seeds each, \texttt{corpus} recipe;
4 seeds for \texttt{boucwen}, after one seed diverged to non-finite weights
during training and was excluded). Table~\ref{tab:loo} shows the answer is
more interesting than either ``representative'' or ``outlier'': the corpus's
five families split into two clear difficulty tiers. \texttt{backlash} and
\texttt{drivetrain} are indistinguishable at $n=5$ seeds at $\sim$0.29--0.30
held-out nMSE --- their means differ by less than either family's
seed-to-seed standard deviation, and their $\pm1\sigma$ bands overlap
substantially --- roughly 6--8$\times$ higher than the other three families
at $\sim$0.035--0.047, which each score \emph{below} the trained-family-average
reference even when held out (Table~\ref{tab:loo}). We note this reference
averages over four families including the two hard ones, so it is an upper
reading of in-distribution difficulty, not each easy family's own
in-distribution nMSE --- the safe reading is that holding these three out is
nearly free, not that it beats training on them. This recontextualizes the
headline cross-family gap (Section~\ref{sec:headline}, Table~\ref{tab:headline}):
the 14--18$\times$ figure is specific to \texttt{backlash}, and cross-family
difficulty is concentrated in two of five families rather than being a
uniform property of family transfer. \texttt{drivetrain}'s difficulty here is
consistent with it already being the hardest family for held-out-\emph{rate}
transfer in Table~\ref{tab:headline} ($8.0\times$ at dt=0.05): its resonant
two-inertia dynamics resist both axes of transfer, not just rate, while
\texttt{backlash}'s discontinuous deadzone is the only other family that
resists family-level holdout this strongly. The corpus's five
families still do not span every control nonlinearity of interest (no
...
\begin{table}[h]
  ...
  reference (mean nMSE over the 4 trained families) vs.\ held-out family
  nMSE, both dt=0.05, ranked by held-out nMSE.}
  \label{tab:loo}
  \centering
  \small
  \begin{tabular}{lrrr}
    ...
    \midrule
    drivetrain & $0.0345\pm0.0006$ & $0.2976\pm0.0110$ & 5 \\
    backlash & $0.0600\pm0.0037$ & $0.2899\pm0.0095$ & 5 \\
    saturate & $0.0823\pm0.0029$ & $0.0474\pm0.0028$ & 5 \\
    stribeck & $0.0833\pm0.0042$ & $0.0388\pm0.0036$ & 5 \\
    boucwen & $0.0776\pm0.0041$ & $0.0351\pm0.0016$ & 4 \\
    \bottomrule
  \end{tabular}
\end{table}
...
capacity-monotonic ordering (wide $<$ deep $<$ narrow $<$ shallow in absolute
...
than a quieter framing would be. A third result, the leave-one-family-out
sweep (Section~\ref{sec:discussion}, Table~\ref{tab:loo}), is likewise
inconvenient rather than confirmatory: it shows our single held-out family
was not representative, and that the headline cross-family gap is specific
to two of five families rather than a general property of family transfer.
We surface it in the same spirit.
...
statistical re-analysis of our identifiability hypothesis, a 5-seed
architecture ablation, and a leave-one-family-out sweep showing cross-family
difficulty is concentrated in two of five families (\texttt{backlash} and
\texttt{drivetrain}), not uniform across families. The corpus's central claim
--- broader training data narrows distribution-shift gaps for in-context
SysID transformers --- holds up under these checks, and the
\texttt{backlash} held-out-family gap specifically holds up as
...
\item {\bf Claims}
  \begin{itemize}
    ...
    \item Justification: The abstract states five concrete, hedged claims: (i) WH-only training collapses off-distribution (cross-family nMSE degradation of $93$--$118\times$); (ii) PLANTFORGE's multi-axis corpus closes the rate/excitation gap for 4 of 5 families but only halves, not closes, the cross-family gap ($93$--$118\times \to 14$--$18\times$); (iii) a classical ARX baseline outperforms both trained transformers on all three real-plant benchmarks by $11$--$342\times$; (iv) identifiability annotations do not positively predict prediction difficulty (median Spearman $r=-0.122$ across 40 cells); (v) full disclosure of a Simpson's-paradox confound in our own first identifiability analysis. Each claim is backed by a corresponding experiment (Sections~\ref{sec:headline}--\ref{sec:identifiability}) and hedged by a matching Limitations paragraph (Section~\ref{sec:discussion}) --- e.g., the cross-family-gap claim explicitly notes it is specific to the \texttt{backlash} holdout, not a general property of family transfer (Section~\ref{sec:headline}, Section~\ref{sec:discussion}, Table~\ref{tab:loo}).
  \end{itemize}
...
\item {\bf Experiment Statistical Significance}
  \begin{itemize}
    ...
    \item Answer: \answerYes{}
    \item Justification: Every headline, real-plant, ablation, and leave-one-family-out number is reported as mean$\pm$std over 5 (4 in one disclosed exception, Table~\ref{tab:loo}'s \texttt{boucwen} row, where one seed diverged to non-finite weights during training and was excluded) independently-seeded training runs. Section~\ref{sec:headline} explicitly discusses the limits of the $\pm1$ standard deviation heuristic used as a comparison bar (a descriptive, not multiplicity-corrected, statistic), and Section~\ref{sec:discussion} separately flags a specific high-variance cell (Bouc-Wen \texttt{wh\_only}, std 0.848 on a mean of 2.537) along with a documented single-seed-vs-5-seed reversal on that exact cell, rather than reporting only means.
  \end{itemize}
