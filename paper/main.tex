...
The training recipe we stress-test --- Wiener-Hammerstein-only white-noise
...
extremely heavy right tail; the quartile \emph{medians} are flat (0.037,
0.037, 0.040, 0.021, Figure~\ref{fig:quartile}) --- our own initial
...
11--342$\times$ margins dwarf plausible window-sampling noise, but this is a
first zero-shot probe, not an exhaustive evaluation. Bouc-Wen's
\texttt{wh\_only} variance (std 0.848 on a mean of 2.537) is our largest
relative real-plant variance, and a single-seed preview of this cell showed
...
\texttt{corpus} recipe; \texttt{boucwen} at $n=4$ after one seed diverged to
non-finite weights) shows the corpus's five families split into two
difficulty tiers rather than one representative-or-outlier case
(Table~\ref{tab:loo}). \texttt{backlash} and \texttt{drivetrain} are
indistinguishable at $\sim$0.29--0.30 held-out nMSE (means differ by less
than either's seed-to-seed std, $\pm1\sigma$ bands overlap) --- 6--8$\times$
higher than the other three families ($\sim$0.035--0.047), which score
\emph{below} the trained-family-average reference when held out (an
upper-bound reference averaged over 4 families including the 2 hard ones,
not each easy family's true in-distribution nMSE --- read as ``nearly free
to hold out,'' not ``beats training on it''). This recontextualizes the
headline cross-family gap (Section~\ref{sec:headline}): the 14--18$\times$
figure is specific to \texttt{backlash}, not a uniform property of family
transfer. \texttt{drivetrain}'s difficulty is consistent with it already
being hardest for held-out-\emph{rate} transfer ($8.0\times$,
...
unchanged. The same $\times$ref ratio trap flagged above (Table~\ref{tab:loo}'s
discussion) and in Section~\ref{sec:identifiability} recurs here: \texttt{deep}'s
worse-looking relative gap ($19.1\times$ vs.\ \texttt{shallow}'s $7.4\times$)
reflects a $3\times$ difference in reference nMSE (0.0158 vs.\ 0.0432), not
absolute family-transfer error (0.3012 vs.\ 0.3198, within either variant's
own std). This check does not cover context/query-length variation; a sweep
over those axes is left to future work.
...
leave-one-family-out sweep. The claim holds up: the \texttt{backlash}
held-out-family gap is capacity-invariant across a 15$\times$ range, and
cross-family difficulty concentrates in two of five families
