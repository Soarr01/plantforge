...
Spearman $r=-0.093$ across 40 cells), the opposite of our working hypothesis; and
...
CRLB on the deadzone width $\sim$5$\times10^4$, versus $\sim$226 for an
excitation that does --- a 233$\times$ gap in the correct direction.
...
40,000 instances and correlated directly: Spearman $r=-0.380$ (max rel-CRLB) and
$r=-0.314$ (log$_{10}$ FIM condition), both statistically significant. Per-family
pooled correlations are uniformly negative but span a wide range
(\texttt{stribeck} $-0.052$ to \texttt{backlash} $-0.380$ and
\texttt{drivetrain} $-0.321$; \texttt{saturate} $-0.160$, \texttt{boucwen}
$-0.091$) --- yet \texttt{backlash}'s own \emph{within-cell-averaged}
correlation (each of its 8 cells correlated separately, then meaned) is
$+0.071$, the \emph{opposite sign} of its own pooled value. This reversal,
entirely internal to one family, is the fingerprint of a Simpson's-paradox
confound: excitation and rate choice change both identifiability and model
difficulty within \texttt{backlash} itself, so pooling across
\texttt{backlash}'s own cells conflates that between-cell structure with the
within-cell effect the hypothesis is actually about. We discarded the pooled
analysis as the primary result and re-ran within-cell.
...
\begin{table}[h]
  ...
  \begin{tabular}{lrrr}
    ...
    \midrule
    base & $-0.093$ & $-0.070$ & 9/40 \\
    power-controlled (excl.\ bottom-decile power) & $-0.075$ & $-0.074$ & --- \\
    range-filtered (IQR$>10^{-6}$, 40/40 cells kept) & $-0.093$ & --- & --- \\
    \bottomrule
  \end{tabular}
\end{table}
...
controls without flipping sign (Table~\ref{tab:ident}). Only 9/40 cells are
positive, concentrated almost entirely in \texttt{backlash} (4/8 cells, family
mean $+0.071$, driven by its \texttt{multisine} and \texttt{closedloop}
excitations); the remaining 5 positive cells are split thinly across
\texttt{saturate} (1/8), \texttt{boucwen} (2/8), and \texttt{drivetrain}
(2/8). \texttt{stribeck} is negative in every one of its 8 cells. $r$ reaches
$+0.352$ at its highest (\texttt{backlash}, \texttt{closedloop}, dt=0.02).
Every other family's mean is negative (\texttt{stribeck} $-0.107$,
\texttt{saturate} $-0.136$, \texttt{boucwen} $-0.091$, \texttt{drivetrain}
$-0.086$, Figure~\ref{fig:spearman}).
...
per-instance nMSE swing erratically across quartiles ($2.2\times10^4 \to
3.3\times10^8 \to 8.2\times10^8 \to 1.1\times10^8$, not even monotonic)
purely because per-instance nMSE has an extremely heavy right tail dominated
by a handful of outliers per bin; the quartile \emph{medians}, by contrast,
decrease monotonically and cleanly (0.083, 0.057, 0.020, 0.018,
Figure~\ref{fig:quartile}) --- consistent with, and a cleaner picture than,
the small negative correlation already reported above. Our own initial
